...
<title><?php echo func::tr('Blocked accounts'); ?></title>
<style>
</style>
...
function clear_form() {
	document.blocked_accounts_form.password.value='';
	if(document.blocked_accounts_form.remember) document.blocked_accounts_form.remember.checked=false;
...
function hash_password() {
	document.blocked_accounts_form.password.value='hashed-'+site_salt+'-'+hex_sha256(site_salt+document.blocked_accounts_form.password.value);
}
...
function validate() {//client side validator
	...
	if(password_exists) if(!document.blocked_accounts_form.password.value.length) msgs[i++]="<?php echo func::tr('password field is empty!'); ?>";

	if(captcha_exists) validate_captcha(document.blocked_accounts_form.captcha.value);
	...
	if(captcha_exists) {
		form_obj=document.blocked_accounts_form;
		check_captcha();
		return false;
	}
...
while($rec=$reg8log_db->fetch_row()) {
	...
	echo '</tr>';
	if(isset($_POST['ext'.$rec['ext_auto']], $_POST[$rec['auto']])) $boths[]=$i;
	else if(isset($_POST['ext'.$rec['ext_auto']])) $unblocks[]=$i;
