...
while($rec=$GLOBALS['reg8log_db']->fetch_row()) {
	...
	if($rec['unblocked']) {
		...
		(
		strtolower($rec['last_username'])!=='admin' and REQUEST_TIME-$rec['first_attempt']<config::get('ip_block_period') and $rec['block_threshold']>=config::get('ip_block_threshold')
		)
		or
		(
		strtolower($rec['last_username'])==='admin' and REQUEST_TIME-$rec['first_attempt']<config::get('admin_ip_block_period') and $rec['block_threshold']>=config::get('admin_ip_block_threshold')
		)

		) {
		echo '<span style="color: red" ';
		echo 'title="', func::tr('Block lift'), ': ', func::duration2friendly_str(config::get('ip_block_period')-(REQUEST_TIME-$rec['first_attempt']), 2), func::tr(' later');
