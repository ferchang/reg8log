...
<title><?php echo func::tr('Accounts awaiting email verification'); ?></title>
<style>
</style>
...
function clear_form() {
	document.pending4email_form.password.value='';
	if(document.pending4email_form.remember) document.pending4email_form.remember.checked=false;
...
function hash_password() {
	document.pending4email_form.password.value='hashed-'+site_salt+'-'+hex_sha256(site_salt+document.pending4email_form.password.value);
}
...
function validate() {//client side validator
	...
	if(password_exists) if(!document.pending4email_form.password.value.length) msgs[i++]="<?php echo func::tr('password field is empty!'); ?>";

	if(captcha_exists) validate_captcha(document.pending4email_form.captcha.value);
	...
	if(captcha_exists) {
		form_obj=document.pending4email_form;
		check_captcha();
		return false;
	}
...
?>
<form action="" method="post" name="pending4email_form">
<?php
...
echo '<th>';
echo "<a class='header' href='?per_page=$per_page&page=$page&sort_by=admin_confirmed&sort_dir=";
if($sort_by==='admin_confirmed' and $sort_dir==='asc') echo 'desc';
else echo 'asc';
echo "'>", func::tr('Needs admin confirmation'), "</a>";
if($sort_by==='admin_confirmed') {
	echo '&nbsp;';
	if($sort_dir==='asc') echo '<img src="../image/sort_asc.gif">';
	else echo '<img src="../image/sort_desc.gif">';
}
...
echo '</td><td align="center" valign=top><input type="button" onclick="check_all(\'del\')" value="', func::tr('All'), '" disabled id="check_all2"></td></tr>';
echo '</table>';

echo '<script>';
echo "document.getElementById('check_all2').disabled=false;\n";
echo '</script>';

require ROOT.'include/page/admin/page_gen_paginated_page_links.php';

$form_name='pending4email_form';
