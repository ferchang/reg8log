...
$query='select sum(`num_dec`) as `num_decs` from `ip_incorrect_logins_decs` where `ip`!='.$ip.' and `account_auto`='.$user->user_info['auto'].' and `timestamp`>'.($req_time-$ip_block_period);
