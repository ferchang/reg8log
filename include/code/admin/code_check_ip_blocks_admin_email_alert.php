...
if(isset($ip_blocks_alert_threshold_reached)) {
	...
	$admin_alert_email_msg='- '.sprintf(func::tr('There were %d new IP block(s).', false, $admin_emails_lang), $new_ip_blocks)."\n";
...
if($admin_alert_email_msg) {
	require ROOT.'include/code/email/admin/code_email_admin_alert_msg.php';
		...
		if(mt_rand(1, floor(1/$cleanup_probability))==1) require ROOT.'include/code/cleanup/code_block_alert_emails_history_expired_cleanup.php';
		if(mt_rand(1, floor(1/$cleanup_probability))==1) require ROOT.'include/code/cleanup/code_block_alert_emails_history_size_cleanup.php';
	}
}
