...
if(!(!config::get('alert_emails_min_interval') or REQUEST_TIME>=($last_alert_email+config::get('alert_emails_min_interval')))) {
	$GLOBALS['reg8log_db']->query("select release_lock('$lock_name3')");
	return;
}
...
if(config::get('max_alert_emails')) {
	...
	if($GLOBALS['reg8log_db']->count_star($query)>=config::get('max_alert_emails')) {
		$GLOBALS['reg8log_db']->query("select release_lock('$lock_name3')");
		return;
	}
}
...
if(isset($ip_blocks_alert_threshold_reached)) {
	...
	$query='update `admin_block_alerts` set `new_ip_blocks`=0, `last_alert`='.REQUEST_TIME." where `for`='email' limit 1";
	$GLOBALS['reg8log_db']->query($query);
