...
$rec6=$GLOBALS['reg8log_db']->fetch_row();
...
if(!(!config::get('registeration_alert_emails_min_interval') or REQUEST_TIME>=($last_reg_alert_email+config::get('registeration_alert_emails_min_interval')))) {
	$GLOBALS['reg8log_db']->query("select release_lock($reg_email_alert_lock)");
	return;
}
...
if(config::get('max_registeration_alert_emails')) {
	...
	if($GLOBALS['reg8log_db']->count_star($query)>=config::get('max_registeration_alert_emails')) {
		$GLOBALS['reg8log_db']->query("select release_lock($reg_email_alert_lock)");
		return;
	}
}
